  ...
  do
     ...
     if (myrow + 1 == slivers(diagsliver)%meshrow .and. mycol == proc) then     
        ...
        if (slivers(diagsliver)%c2g == slivers(diagsliver)%r1g .and. slivers(diagsliver)%height ==1) then
           ...
           call dgebr2d(ictxt, 'C', ' ', 1, slivers(diagsliver)%c2g - slivers(diagsliver)%c1g + 1, cvec(slivers(diagsliver)%c1g), 1, &
                slivers(diagsliver)%meshrow - 1, proc)
           call dgebr2d(ictxt, 'C', ' ', 1, slivers(diagsliver)%c2g - slivers(diagsliver)%c1g + 1, svec(slivers(diagsliver)%c1g), 1, &
                slivers(diagsliver)%meshrow - 1, proc)
        end if
     end if
